...
export default function App() {
  ...
  function PortfolioView() {
    return (
      <div style={{ height: "100%", overflowY: "auto", padding: isMobile ? 16 : 24 }}>
        <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 22, color: C.text, marginBottom: 20 }}>Portfolio Overview</div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
              {portfolio.map(t => (
                <div key={t} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 20, padding: "4px 12px 4px 14px", fontFamily: "'JetBrains Mono',monospace", fontSize: 12, color: C.text }}>
                  {t}
                  <button onClick={() => removeFromPortfolio(t)} style={{ background: "none", border: "none", color: C.textSec, padding: 0, display: "flex", alignItems: "center", cursor: "pointer", transition: "color 150ms" }}
                    onMouseEnter={e => e.target.style.color = C.neg}
        ...
        {portfolio.length === 0 && !portfolioData && (
          <EmptyState Icon={PieChart} title="No tickers added." subtitle="Add tickers above to begin portfolio analysis." />
        )}
              ...
              <>
                ...
                <div className="port-grid">
                  {Object.entries(portfolioData.breakdown).map(([ticker, info], idx) => {
                    ...
                    const sym    = ticker.endsWith(".NS") || ticker.endsWith(".BO") ? "₹" : "$";
                    return (
