...
# ── Cache setup ────────────────────────────────────────
stock_cache     = TTLCache(maxsize=50,  ttl=300)    # 5 min
news_cache      = TTLCache(maxsize=100, ttl=300)    # 5 min
earnings_cache  = TTLCache(maxsize=50,  ttl=3600)   # 60 min
taketoday_cache = TTLCache(maxsize=50,  ttl=600)    # 10 min
...
COMPANY_MAP = {
    ...
    "visa": "V", "mastercard": "MA", "reliance": "RELIANCE.NS",
...
def extract_ticker(query: str) -> str | None:
    ...
    # Longest match first to avoid partial matches (jp morgan before morgan)
...
def build_context(query: str, time_range: str = "7d") -> str:
    ...
    # TakeToday first — highest credibility, biases LLM toward verified info
    taketoday = get_taketoday_news(query)
    news = get_financial_news(query, ticker, time_range=time_range)
